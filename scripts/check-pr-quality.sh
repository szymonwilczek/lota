...
usage() {
	cat >&2 <<'USAGE'
usage: scripts/check-pr-quality.sh <base-commit> <head-commit>
...
check_hotpath_docs() {
	...
	while IFS='|' read -r hot_globs doc_globs reason; do
		...
		if ((${#hot_hits[@]} > 0 && ${#doc_hits[@]} == 0)); then
			printf 'hotpath changed without same-commit docs: %s %s\n' \
				"$(short_commit "$commit")" "$(commit_subject "$commit")" >&2
			printf 'reason: %s\n' "$reason" >&2
...
main() {
	...
	if ((ai_assisted == 1)); then
		printf 'PR quality gate passed for %d commit(s); AI-assisted marker found.\n' \
			"${#commits[@]}"
	else
		printf 'PR quality gate passed for %d commit(s).\n' "${#commits[@]}"
	fi
}
