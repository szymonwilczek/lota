...
func TestEnrollFullCeremony(t *testing.T) {
	...
	ch, err := svc.Begin(ek.CertDER, aikTPMT, nil, nil)
	if err != nil {
...
func TestEnrollWrongEKCannotActivate(t *testing.T) {
	...
	ch, err := svc.Begin(issueEK.CertDER, aikTPMT, nil, nil)
	if err != nil {
		t.Fatalf("Begin: %v", err)
	}
...
func TestEnrollMaxPending(t *testing.T) {
	...
	if _, err := svc.Begin(ek.CertDER, aik1, nil, nil); err != nil {
		t.Fatalf("first Begin: %v", err)
	}
	aik2, _ := tpmtest.AIKTemplate(t)
	if _, err := svc.Begin(ek.CertDER, aik2, nil, nil); err == nil {
