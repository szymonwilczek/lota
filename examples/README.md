...
| `ha-postgres/`     | Multi-instance verifier on a shared Postgres backend: two instances share enforcement and session-token state via `--pg-dsn`, so a token issued by one validates on the other. |
