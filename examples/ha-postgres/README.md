...
state: a token issued by one instance validates on another, and a revocation
or ban on one is enforced by all. This example stands two instances up against
one Postgres so the shared-state behaviour can be observed without a full
fleet.

See [`docs/HA_DEPLOYMENT.md`](../../docs/HA_DEPLOYMENT.md) for the topology and
the shared-state requirements, and [`examples/enrollment/`](../enrollment) for
the CA + attestation ceremony this example reuses.
...
Each instance is stateless and identical apart from its listen ports. Both
take `--pg-dsn` (here via `LOTA_PG_DSN`), the same CA root, and the same
policy, so they enforce the same rules over the same shared state.
...
`consume:true` call marks the token used in Postgres, so a second
validation of the same token on **either** instance reports `consumed:true` --
...
- uses real TLS material and `sslmode=verify-full` to a least-privilege
  Postgres role;
- keeps the CA signing key in an HSM (see [`docs/CA-KEY.md`](../../docs/CA-KEY.md)).
