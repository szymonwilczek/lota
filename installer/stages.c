				       ...
				       size_t cap)
{
...

	{
		const char *const argv[] = {
		    PATH_AGENT_BIN,    "--verify-policy",	PATH_BPF_OBJ,
		    "--policy-pubkey", ctx->opts.policy_pubkey, NULL};
...
static int st_initrd_apply(struct install_ctx *ctx)
{
	...
	int rc =
	    run_cmd(&ctx->ui, "Regenerating the initramfs (dracut -f)", argv);
					 ...
					 size_t cap)
{
	...
	if (grubby_default_args(args, sizeof(args)) == 0) {
		...
		int sig_cfg = st.sig_ok || probe_cmdline_has_token(
					       args, "module.sig_enforce=1");
		int lock_cfg =
		    st.lockdown_ok ||
...
static int st_selinux_apply(struct install_ctx *ctx)
{
	...
	{
		const char *const argv[] = {"udevadm", "trigger", "/dev/tpmrm0",
					    "/dev/tpm0", NULL};

		rc =
		    run_cmd(&ctx->ui, "Re-labeling the TPM device nodes", argv);
		if (rc != 0)
			return rc > 0 ? -EIO : rc;
	}
	{
		const char *const argv[] = {"restorecon", PATH_AGENT_BIN, NULL};

		rc =
		    run_cmd(&ctx->ui, "Restoring the agent binary label", argv);
		if (rc != 0)
			return rc > 0 ? -EIO : rc;
	}
	return 0;
...
static int st_agent_apply(struct install_ctx *ctx)
{
	...
	{
		const char *const argv[] = {"systemctl", "daemon-reload", NULL};
...
const struct stage install_stages[] = {
	...
	.explain = "fs-verity makes the kernel refuse any modified read of "
	...
	.explain = "The agent refuses to start unless the kernel enforces an "
	...
	.explain = "udev rule labels /dev/tpm* with a LOTA-specific SELinux "
